...
use aionui_db::{IAgentMetadataRepository, SqliteAgentMetadataRepository, init_database};
...
pub async fn run_doctor(cli: &Cli, merged_path: &str) -> Result<ExitCode, CliBoundaryError> {
    ...
    let db_path = cli.data_dir.join("saydone.db");
...
mod tests {
    ...
    fn doctor_database_error_uses_stable_code_without_raw_path() {
        ...
        assert!(!err.stderr_line().contains("/Users/secret/saydone.db"));
    }
