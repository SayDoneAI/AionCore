...
impl BackendConnection for ClaudeConnection {
    async fn open_session(
        ...
        // legacy title state disarmed so a fresh Claude session never sends
        // `generate_session_title` on the user's connection.
        let backend = ClaudeSessionBackend::spawn(logical_id, adapter, io, config, wake, false).await;
