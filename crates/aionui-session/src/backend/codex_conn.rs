...
use super::codex_title::{NoTitleIo, TitleGen};
...
impl BackendConnection for CodexConnection {
    async fn open_session(
        ...
        // Conversation titles are generated by the SayDone client now. Keep the
        // legacy latch disarmed and give it a source that can never spawn a
        // process, so a fresh Codex session cannot start a title app-server.
        let title_gen = Arc::new(TitleGen::new(false, Arc::new(NoTitleIo)));
